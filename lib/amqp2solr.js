...
var self = function(dependencies) {
  ...
  rabbitmq.on('ready', function() {
    ...
    _.each(internalQueue, function(q) {
      publish(q.action, q.queue, q.payload);
    });
    internalQueue = [];
  });
    ...
    if (ready) {
      rabbitmq.queue(queueName, persistentQueue, function(q) {
        ...
        q.subscribe(function(message, headers, deliveryInfo) {
          logger.debug('DAT MESSAGE APPROACHES');
          solrResource[message.action](message.payload, function() {
            logger.debug('DAT WTF', arguments);
          });
  ...
  function publish(action, queue, payload) {
    if (ready) {
      ...
      internalQueue.push({ queue: queue, action: action, payload: payload });
    }
    logger.info({ready: ready, action: action, queue: queue, payload: payload});
  }

  return {
    listen: listen,
    publish: publish,
    add: _.partial(publish, 'add'),
    update: _.partial(publish, 'add'),
    getResource: require('./solrResourceFactory')
  };
};
