...
var self = function(dependencies) {
  ...
  function publish(action, queue) {
    ...
    logger.info({ready: ready, action: action, queue: queue, args: args });
  }


  var publishableActions = [
    'add',
    'findAndModify',
    'update',
    'delete',
  ...
  return _(publishableActions).zipObject().mapValues(function(v, action) {
    return _.partial(publish, action);
  }).assign({
    listen: listen,
    publish: publish,
